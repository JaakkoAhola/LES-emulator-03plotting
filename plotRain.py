...
class ManuscriptFigures(EmulatorMetaData):
    ...
    def _initRainTicks(self):
        self.sbTicks = numpy.arange(0, 60 + 1, 20)
        self.salsaTicks = numpy.arange(0, 125 + 1, 25)
    ...
    def _initReadFeatureImportanceData_phase04(self):
        ...
        try:
            self.labelCategorised = self.labelCategorised.drop(index="rainProduction_linearFit")
        except:
            pass
    ...
    def figureBarFeatureImportanceData(self):
        ...
        for ind in range(nrows*ncols):
            ...
            if ind == 1:
                ...
                PlotTweak.setAnnotation(ax, "Gaussian Process Rain Production Emulator", xPosition=-0.7, yPosition = 1.05,  xycoords = "axes fraction")
    ...
    def figurePredictorsVsSimulated(self):
        ...
        for ind,trainingSet in enumerate(self.trainingSetList):
            ...
            dataframe[self.responseVariable] = dataframe[self.responseVariable] * 1e6
            dataframe[self.emulatedVariable] = dataframe[self.emulatedVariable] * 1e6
...
{PlotTweak.getLatexLabel(f'RMSE={rmse:.4e}','')}""",
            ...
            if "3" in trainingSet:
                ax.set_xticks(self.sbTicks)
                ax.set_xticklabels(self.sbTicksLabels)
                ax.set_xlim([0, self.sbTicks[-1]])
                
                ax.set_yticks(self.sbTicks)
                ax.set_yticklabels(self.sbTicksLabels)
                ax.set_ylim([0, self.sbTicks[-1]])
            elif "4" in trainingSet:
                ax.set_xticks(self.salsaTicks)
                ax.set_xticklabels(self.salsaTicksLabels)
                ax.set_xlim([0, self.salsaTicks[-1]])
                
                ax.set_yticks(self.salsaTicks)
                ax.set_yticklabels(self.salsaTicksLabels)
                ax.set_ylim([0, self.salsaTicks[-1]])
        ...
        for ind,trainingSet in enumerate(self.trainingSetList):
            ...
            if ind == 2:
                ax.text(PlotTweak.getXPosition(ax, -0.32), PlotTweak.getYPosition(ax, 0.5),
                        PlotTweak.getUnitLabel("Predicted\ Rain\ production", PlotTweak.getVariableUnit("rainProduction")), size=8 , rotation =90)
            if ind == 3:
                ax.text(PlotTweak.getXPosition(ax, -0.7), PlotTweak.getYPosition(ax, -0.22),PlotTweak.getUnitLabel("Simulated\ Rain\ Production", PlotTweak.getVariableUnit("rainProduction")) , size=8)
    ...
    def figureRainDistributions(self):
        nrows = 2
        ncols = 2
        
        self.figures["figureRainDistributions"] = Figure(self.figureFolder,"figureRainDistributions",
                                                   figsize = [self.figureWidth, 7],  ncols = ncols, nrows = nrows,
                                                   bottom = 0.08, hspace = 0.17, wspace=0.35, top=0.95, left=0.12, right = 0.95)
        fig = self.figures["figureRainDistributions"]
        ...
        for ind,trainingSet in enumerate(self.trainingSetList):
            ...
            if hasattr(self, "filteredSourceData") and (self.filteredSourceData is not None):
                self.filteredSourceData["raindProduction"] = self.filteredSourceData.apply(lambda row: row["accretion1_int"] + row["accretion2"] + row["autoconv"], axis = 1)
                self.filteredSourceData["raindProduction"] = self.filteredSourceData["raindProduction"]*1e6
                print("all echam columns: ", len(self.filteredSourceData["raindProduction"]))
                # sourceDataVariable = self.filteredSourceData["raindProduction"]
                ...
                sourceDataVariable = filteredSourceData_TimeOfDay["raindProduction"]
                print(f'{timeOfDay} {len(filteredSourceData_TimeOfDay)/len(self.filteredSourceData["raindProduction"]):.2f}' )
            ...
            simulated = dataframe[self.responseVariable] *1e6
            emulated = dataframe[self.emulatedVariable] *1e6
            ...
            if "3" in trainingSet:
                ...
                # ax.set_yticks(self.sbDensityTicks)
                # ax.set_yticklabels(self.sbDensityTicksLabels)
                # ax.set_ylim([0, self.sbDensityTicks[-1]])
            elif "4" in trainingSet:
                ax.set_xticks(self.salsaTicks)
                ax.set_xticklabels(self.salsaTicksLabels)
                ax.set_xlim([0, self.salsaTicks[-1]])
                
                # ax.set_yticks(self.salsaDensityTicks)
                # ax.set_yticklabels(self.salsaDensityTicksLabels)
                # ax.set_ylim([0, self.salsaDensityTicks[-1]])
        ...
        for ind,variable in enumerate(self.trainingSetList):
            ...
            if ind == 1:
                PlotTweak.setAnnotation(ax, PlotTweak.getUnitLabel("Rain\ Production\ Distributions", PlotTweak.getVariableUnit("rainProduction")), xPosition=-0.9, yPosition = 1.05,  xycoords = "axes fraction")
...
def main():
    try:
        ...
        locationsFile = "/home/aholaj/mounttauskansiot/puhtiwork/RainEmulator/rainLocationsMounted.yaml"
    ...
    if True:
        ...
        figObject.figureRainDistributions()
